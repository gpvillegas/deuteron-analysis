...
Make DC time vs wirenum plots for HMS
"""
...
dc_branches = D.get_list(db.retrieve(deut_db_name, 'Branches', 'TTree_Branches',
                          where='Branches like \'H.dc.%.time\'')) +\
                D.get_list(db.retrieve(deut_db_name, 'Branches', 'TTree_Branches',
                          where='Branches like \'H.dc.%.wirenum\''))
#%%
...
for r in runs:
    ...
    for plane in dc_plane_names:
        time_name = f'H.dc.{plane}.time'
        wirenum_name = f'H.dc.{plane}.wirenum'
...
for r in runs:    
    fig = B.pl.figure(layout='constrained',figsize=(22,10))
    fig.suptitle(f'HMS Drift time vs. Wire number\n Run {r}', fontsize=16)
    rows = 2
    cols = 6
    i = 1
    for plane in dc_plane_names:
    ...
    #B.pl.savefig(f'./hms_dc_calib/plots/htime_wirenum_{r}.png')
